    ...
    runDesc *run;
{
    ...
    /* fflush(run->fp); POR MMC - innecesario con MEMFILEs */        /* Gotta do this for LATTICE. */
    if ((run->pointPos = memftell(run->fp)) <= 0)
	run->pointPos = i;

    memfprintf(run->fp, "0       \n"); /* Save 8 spaces here. */

    memfprintf(run->fp, "Command: version %s\n", ft_sim->version);
    memfprintf(run->fp, "Variables:\n");
    ...
    runDesc *run;
{
    ...
    for (i = 0; i < run->numData; i++) {
        ...
        memfprintf(run->fp, "\t%d\t%s\t%s", i, name,
                ft_typenames(type));
	if (run->data[i].gtype == GRID_XLOG)
	    memfprintf(run->fp, "\tgrid=3");
	memfprintf(run->fp, "\n");
...
{
    if (bin)
        memfwrite((char *) &value, sizeof (double), 1, fp);
    else
    ...
    IFcomplex value;
{
    ...
    if (bin) {
        memfwrite((char *) &value.real, sizeof (double), 1, fp);
        memfwrite((char *) &value.imag, sizeof (double), 1, fp);
    } else {
...
fileEnd(run)
    runDesc *run;
{
/* POR MMC - no sirve en nuestro caso */
# if 0
	
	long place;

    /*if (run->fp != stdout) { POR MMC - asegurado con MEMFILEs */
	place = memftell(run->fp);
	memfseek(run->fp, run->pointPos, 0);
	memfprintf(run->fp, "%d", run->pointCount);
	memfseek(run->fp, place, 0);
    /*} else {
	/* Yet another hack-around *//*
	fprintf(stderr, "@@@ %ld %d\n", run->pointPos, run->pointCount);
    }*/
    /* fflush(run->fp); PRO MMC - innecesario con MEMFILEs */
