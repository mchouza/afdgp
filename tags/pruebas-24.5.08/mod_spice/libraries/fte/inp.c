 ...
 */
#if 0
Ésta es la versión en disco, creo una qiue lea desde memoria
void
...
void
inp_spsource_from_mem(const char* nl, bool comfile)
{
    struct line *deck, *dd, *ld;
    struct line *realdeck, *options;
    char *tt, name[BSIZE_SP], *s, *t;
        ...
        if (deck->li_next) {
            ...
            if (!cp_getvar("nosubckt", VT_BOOL, (char *) 
                ...
                deck->li_next = inp_subcktexpand(deck->
                        li_next);
            deck->li_actual = realdeck;
            inp_dodeck(deck, tt, wl_first, false, options, NULL);
        }
