...
/* Hago lo mismo que readline() pero desde memoria en lugar de un archivo */
static char *
readline_from_mem(const char* mem, int* mem_pos)
{
    int c;
    int memlen;
    char *strptr;
    int strlen;
	int i; /* Por mmc para avanzar por memoria */
    
    i = *mem_pos; /* Por mmc, empiezo en mem_pos */
	strptr = NULL;
    strlen = 0;
    memlen = STRGROW; 
    strptr = tmalloc(memlen);
    memlen -= 1;          /* Save constant -1's in while loop */
    while((c = mem[i++]) != '\0') { /* Por mmc, no tengo EOF pero si '\0' */
	if (strlen == 0 && c == '\n')
	    continue;
        strptr[strlen] = c;
        strlen++;
    ...
    char *name, *mode;
{
    ...
    while (v) {
		...
		(void) sprintf(buf, "%lg%s%s", v->va_real, DIR_PATHSEP, name);
		break;
        }
...
#if 0
Creo una que lea desde memoria
void
inp_readall(fp, data)
    FILE *fp;
    struct line **data;
...
void inp_readall_from_mem(const char* nl_mem, struct line **data)
{
    struct line *cc, *end = NULL, *prev = NULL, *working, *newcard;
    char *buffer, *s, *t, c;
    int line = 1;
    /*FILE *newfp;*/
	int mem_pos = 0; /* Para evitar leer siempre la misma línea */

    while (buffer = readline_from_mem(nl_mem, &mem_pos)) {
        if (*buffer == '@')
            break;
        for (s = buffer; *s && (*s != '\n'); s++)
            ;
...
#if 0
		Desactivo el include, ya que no tiene sentido si leemos de memoria
		if (ciprefix(".include", buffer)) {
            for (s = buffer; *s && !isspace(*s); s++)
                ;
            while (isspace(*s))
